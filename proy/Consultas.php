			...
			if ($id == "ingredientes") {
				...
				if (mysqli_num_rows($result) > 0 ) {
					...
					while ($row = mysqli_fetch_assoc($result)) {
						...
						echo "<td>" . $grupo . "</td>";
						echo "<td style='text-align: right;     padding: 0px 0px; width:10%; '><a onclick='showModifyModal(&quot;".$row["NombreIngrediente"]."&quot;,".$row["GrupoAlimenticio"].", &quot;".$grupo."&quot; )' href='#modifyModal' id='".$row["IDIngrediente"]."' style='display: block;' class='right  waves-effect waves-grey btn-flat grey-text modal-trigger'><i class='material-icons'>create</i></a>"
							;
